...
function buscarOrdenesTrabajo()
{
	...
	$.ajax({
		...
		type:  'post',
		dataType:'json',
		data: {'desea': 'cargaOrdenesFiltros',
...
function cargarDetalleActividades()
{
	 $('#table_actividades').DataTable({        
        ...
        columns: [
			...
			{ data: "ValorHoraMecanico",   title: "Valor Hora Mecánico" },
			{ data: "ValorTotal",   title: "Valor Total" },
			{ data: "Utilidad", title: "% Utilidad" },
			{ data: "ValorTotalUtilidad", title: "Valor Total Utilidad"	},
            { data: "Fecha", title: "Fecha"},
			{ data: "Observaciones", title: "Observaciones"},
			{ data: "EstadoId", title: "EstadoId"},
			{ data: "Estado", title: "Estado"}
	...
	$('#table_actividades').on('click', 'td .A_editar', function (e) {
		...
		$.ajax({
			...
			data: {'desea': 'editarActividad', 
				   ...
				   "fecha": dataItem.Fecha, 
				   "tb_valorHoraMecanico": dataItem.ValorHoraMecanico,		
				   "tb_valorTotalUtilidad": dataItem.ValorTotalUtilidad,		   
				   "tb_valorTotal": dataItem.ValorTotal,
				   "TB_utilidad": $.trim(dataItem.Utilidad),
				...
				$.ajax({
					...
					success:  function (data) {  
						...
						{							
							...
							if(($("#TB_utilidad").val() == undefined) || $("#TB_utilidad").val() ==""  )
							{								
								$("#TB_utilidad").val(0);
							}
...
{
	var valorIni = $("#TB_valorHora").val();
	var tiempo = ($("#TB_tiempo").val()).replace(",", ".");
	
	var valorH = (parseFloat(valorIni) * parseFloat( tiempo ));				
	
	var utilidad = (parseFloat($("#TB_utilidad").val()));
	if(utilidad == undefined)
	{
		utilidad =0;
	}
	var utilidadF = (( utilidad * valorH)/100);
	
	
	
	var number = (valorH + utilidadF)	,
	thousand_separator = ',',
	decimal_separator = '.';
...
function guardarActividad()
{
    ...
    var TB_valorHora = $("#TB_valorHora").val();
	...
	else if($.trim(TB_observaciones) == "")
    {
		...
		TB_valorHora = TB_valorHora.replace(",","");
		TB_tiempo = TB_tiempo.replace(",",".");
		$.ajax({
			url:   '../Logica de presentacion/Orden_Trabajo_Logica.php',
			type:  'post',
			dataType:'html',
